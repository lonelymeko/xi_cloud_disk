...
#### 🎉 功能更新
- **数据看板全面切换为真实数据**：移除首页与文件资源管理器中的模拟数据入口，统一改为真实接口数据
  - 首页看板固定走后端文件列表接口，支持分页汇总后再统计类型占比与数量
  - 文件资源管理器移除 mock 数据通道，保留真实数据 + 刷新 + 滚动加载

#### 🔧 体验优化
- **上传任务名称与文件名称一致**：上传时前端显式提交 `name/ext/size` 元信息，任务状态展示名称与用户上传名称保持一致
- **下载文件名一致性优化**：前端下载使用 `download` 文件名提示，优先按列表显示名称保存

#### 📌 说明
- 看板统计目前基于现有文件列表接口口径（当前目录/分页聚合）。如需“全目录递归统计”，可扩展专用聚合接口。
